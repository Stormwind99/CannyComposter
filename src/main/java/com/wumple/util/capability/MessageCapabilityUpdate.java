...
public abstract class MessageCapabilityUpdate<HANDLER, DATA> implements IMessage
{
    ...
    public void fromBytes(ByteBuf buf)
    {
         ...
         * NBTBase nbt = null;
         * 
         * try { nbt = CompressedStreamTools.read(new ByteBufInputStream(buf), new NBTSizeTracker(2097152L)); } catch (IOException e) {
         * 
         * } finally { provider.getCapability().readNBT(provider.getInstance(), provider.getFacing(), nbt); }
         */
    }
    ...
    public void toBytes(ByteBuf buf)
    {
         ...
         * 
         * try { CompressedStreamTools.writeTag(nbt, new ByteBufOutputStream(buf)); } catch (IOException e) { }
         */
    }
